//#define UICAPNP_H   // Button touch enable for the ALCA and SND
#ifdef UICAPNP_H
...
void bb_ui_draw_UI( UIState *s) {
  ...
  if  (bb_currentTimeInMilis() - s->b.tri_state_switch_last_read > 2000)  {
    ...
    if (tri_state_fd == -1) {
      s->b.tri_state_switch = 1; // Fix for LEON, which don't have a tri-state switch
    } else {
...
void bb_ui_init(UIState *s) {
    ...
    strcpy(s->b.car_model,"Toyota");
    strcpy(s->b.car_folder,"toyota");
...
void  bb_ui_poll_update( UIState *s) {
    ...
    // Erich mod
    if (!s->b.intrinsic_matrix_loaded) {
      s->b.intrinsic_matrix_loaded = try_load_intrinsics(&s->b.intrinsic_matrix);
    }
    //Erich end
